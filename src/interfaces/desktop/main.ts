...
import { PerceptionCascade } from "../../application/observation/perception-cascade.js";
import { WindowsPerception } from "../../infrastructure/windows/windows-perception.js";
...
import { createOperatingContracts } from "../../infrastructure/contracts/operating-contracts.js";
import { OperatingSystemController } from "../../application/operating-system/operating-system.js";
import { CodexOperatingPlanner } from "../../application/planning/codex-planner.js";
import { SqliteOperatingStore } from "../../infrastructure/persistence/sqlite-operating-store.js";
import { CHECKPOINT_STATUSES, DAY_OUTCOMES, EVIDENCE_SOURCES, INTERVENTION_FEEDBACK_KINDS } from "../../core/operating-system/types.js";
...
let controller: OperatingSystemController;
let store: SqliteOperatingStore | null = null;
...
function createKovacsWindow(): BrowserWindow {
  const window = new BrowserWindow({
    ...
    webPreferences: {
      preload: path.join(config.applicationRoot, "ui", "desktop", "preload.cjs"),
  ...
  void window.loadFile(path.join(config.applicationRoot, "ui", "desktop", "index.html"));
  window.once("ready-to-show", () => window.show());
  return window;
}

async function bootstrap(): Promise<void> {
  const [ambientContracts, operatingContracts] = await Promise.all([
    createObservationContracts(config.contractsDirectory), createOperatingContracts(config.contractsDirectory),
  ]);
  const ambientStore = new ObservationStateStore(config.observation.dataDirectory, ambientContracts);
  const settings = await ambientStore.loadSettings(config.observation.settings);
  const service = await KovacsService.create(config.observation.coaching);
  store = await SqliteOperatingStore.create(config.databasePath, operatingContracts);
  ...
  const retrievalByContext = new Map<string, { query: string; project: string | null; results: ReturnType<SqliteOperatingStore["searchMemories"]>; observedAt: number }>();
  let operating: OperatingSystemController;
  const ambient = new ObservationController(service, ambientStore, settings, new WindowsActiveWindowProbe(), new ElectronWindowCapture(), {
  ...
  operating = new OperatingSystemController(ambient, store, new CodexOperatingPlanner(config, operatingContracts));
