...
import { createAmbientContracts } from "../contracts.js";
import { AmbientStateStore } from "../state-store.js";
import { AmbientController } from "../controller.js";
import { ElectronWindowCapture, WindowsActiveWindowProbe } from "./adapters.js";
import type { AmbientStatus } from "../types.js";
...
function createPet(): BrowserWindow {
  const window = new BrowserWindow({
    ...
    webPreferences: {
      preload: path.join(config.applicationRoot, "ui", "v0.2", "preload.cjs"),
  ...
  void window.loadFile(path.join(config.applicationRoot, "ui", "v0.2", "index.html"));
  window.once("ready-to-show", () => window.show());
  return window;
}

async function bootstrap(): Promise<void> {
  const contracts = await createAmbientContracts(config.contractsDirectory);
  const store = new AmbientStateStore(config.dataDirectory, contracts);
  const settings = await store.loadSettings(config.settings);
  const service = await KovacsService.create(config.v01);
  controller = new AmbientController(service, store, settings, new WindowsActiveWindowProbe(), new ElectronWindowCapture());
