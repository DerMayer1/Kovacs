...
import type { ProfileResponse } from "../../core/coaching/types.js";
import type { ObservationController } from "../observation/observation-controller.js";
import type { AmbientReasoningTelemetry, AmbientState, AmbientStatus } from "../../core/observation/types.js";
import type { SqliteOperatingStore } from "../../infrastructure/persistence/sqlite-operating-store.js";
import type {
...
export class OperatingSystemController {
  private listeners = new Set<(update: OperatingUpdate) => void>();
  private planning = false;

  constructor(private readonly ambient: ObservationController, private readonly store: SqliteOperatingStore, private readonly planner: OperatingPlanner) {
  ...
  onUpdate(listener: (update: OperatingUpdate) => void): () => void { this.listeners.add(listener); return () => this.listeners.delete(listener); }
  ...
  private emit(message: string, response?: ProfileResponse): void {
    const update: OperatingUpdate = { ambient: this.ambient.getState(), operating: this.store.snapshot(), message, ...(response ? { response } : {}) };
