...
import { createAmbientContracts, type AmbientContracts } from "../src/v02/contracts.js";
import { defaultAmbientSettings } from "../src/v02/config.js";
import type { AmbientEvent, AmbientState } from "../src/v02/types.js";

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const contracts: AmbientContracts = await createAmbientContracts(path.join(root, "contracts"));
...
const required = ["docs/v0.2/00_CHARTER.md","docs/v0.2/01_ARCHITECTURE.md","docs/v0.2/02_SUCCESS_METRICS.md","contracts/v0.2/ambient-state.schema.json","contracts/v0.2/ambient-event.schema.json","contracts/v0.2/settings.schema.json","src/v02/controller.ts","src/v02/electron/main.ts","ui/v0.2/index.html"];
...
const controller=await readFile(path.join(root,"src/v02/controller.ts"),"utf8");
const permissions=await readFile(path.join(root,"src/v02/permissions.ts"),"utf8");
const policy=await readFile(path.join(root,"src/v02/policy.ts"),"utf8");
const gateway=await readFile(path.join(root,"src/v01/codex-exec.ts"),"utf8");
const electron=await readFile(path.join(root,"src/v02/electron/main.ts"),"utf8");
const preload=await readFile(path.join(root,"ui/v0.2/preload.cjs"),"utf8");
