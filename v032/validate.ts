...
const files = ["src/v03/types.ts", "src/v03/store.ts", "src/v03/controller.ts", "src/v03/planner.ts", "src/v03/electron/main.ts", "src/v02/controller.ts", "src/v02/context-decision.ts", "src/v032/context-engine.ts", "src/v032/memory.ts", "src/v032/perception-cascade.ts", "src/v032/windows-perception.ts", "ui/v0.3/index.html", "ui/v0.3/renderer.js", "ui/v0.3/preload.cjs", "README.md"];
...
record("C07", content["src/v032/memory.ts"].includes("embedLocally") && content["src/v03/store.ts"].includes("searchMemories") && content["src/v03/store.ts"].includes("fts_vector"), "vector memory and hybrid retrieval remain deterministic and local");
...
record("C13", content["src/v032/perception-cascade.ts"].indexOf("readAccessibility") < content["src/v032/perception-cascade.ts"].indexOf("input.capture") && content["src/v032/perception-cascade.ts"].indexOf("input.capture") < content["src/v032/perception-cascade.ts"].indexOf("readOcr") && content["src/v032/perception-cascade.ts"].includes("deterministicTrigger"), "perception order is UIA, then lazy OCR capture, with screenshot escalation limited to unresolved or deterministic failure context");
record("C14", content["src/v02/context-decision.ts"].includes("perception.confidence < 0.65") && content["src/v02/context-decision.ts"].includes("perception.confidence < 0.8") && content["src/v02/context-decision.ts"].includes("candidateSamples < 2") && content["src/v02/context-decision.ts"].includes("sameContextCooldownMs"), "confidence, stability, semantic delta, and same-context cooldown are deterministic");
record("C15", content["src/v03/electron/main.ts"].includes("workingFrames") && content["src/v03/electron/main.ts"].includes("onWorkingContextCleared") && content["src/v03/store.ts"].includes("context_events") && content["src/v03/store.ts"].includes("retention_class='event'"), "working context is volatile and durable context is event-bound with explicit retention");
record("C16", content["ui/v0.3/index.html"].includes("context-view") && content["ui/v0.3/renderer.js"].includes("renderContextDiagnostics") && content["src/v03/types.ts"].includes("context_diagnostics"), "context decisions are inspectable in a separate diagnostics surface");
record("C17", content["src/v02/context-decision.ts"].includes("wrong_context") && content["src/v03/controller.ts"].includes("recordContextFeedback") && content["src/v02/context-decision.ts"].includes("suppressedFingerprints"), "wrong-context feedback suppresses repetition for the session without autonomous adaptation");
