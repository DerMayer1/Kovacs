...
try {
  ...
  store = await SqliteOperatingStore.create(path.join(temporary, "data", "kovacs.db"), contracts);
  ...
  store.recordContextDecision({ occurred_at: new Date().toISOString(), context_id: frame.context_id, application: frame.application,
    ...
    image_attached: false, sensitive_categories: [], screenshot_blocked_reason: null, bypass_global_cooldown: true });
