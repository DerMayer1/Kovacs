...
import { createOperatingContracts, type OperatingContracts } from "../../../src/infrastructure/contracts/operating-contracts.js";
import type { DayProposal, EvidenceRecord, MemoryRecord, SetupProposal, WeekProposal } from "../../../src/core/operating-system/types.js";
...
const required = [
  ...
  "src/application/operating-system/operating-system.ts", "src/infrastructure/persistence/sqlite-operating-store.ts", "src/application/planning/codex-planner.ts", "src/interfaces/desktop/main.ts", "ui/desktop/index.html", "ui/desktop/preload.cjs",
];
...
const contracts: OperatingContracts = await createOperatingContracts(path.join(root, "contracts"));
...
const evidence: EvidenceRecord = { schema_version: "0.3.0", evidence_id: `ev_${"a".repeat(32)}`, day_id: `day_${"b".repeat(32)}`, checkpoint_id: `cp_${"c".repeat(32)}`, project: root, competency: "testing_reliability", source: "tool_verified", assistance_level: "A1", outcome: "achieved", confidence: 0.9, summary: "Focused tests passed", validation: "Test output", source_event_id: null, created_at: at };
...
const store = await readFile(path.join(root, "src/infrastructure/persistence/sqlite-operating-store.ts"), "utf8");
const controller = await readFile(path.join(root, "src/application/operating-system/operating-system.ts"), "utf8");
const planner = await readFile(path.join(root, "src/application/planning/codex-planner.ts"), "utf8");
const gateway = await readFile(path.join(root, "src/infrastructure/codex/codex-exec-gateway.ts"), "utf8");
const electron = await readFile(path.join(root, "src/interfaces/desktop/main.ts"), "utf8");
const preload = await readFile(path.join(root, "ui/desktop/preload.cjs"), "utf8");
const tests = await readFile(path.join(root, "test/operating-system/operating-system.test.ts"), "utf8");
const charter = await readFile(path.join(root, "docs/v0.3/00_CHARTER.md"), "utf8");
const ambientTypes = await readFile(path.join(root, "src/core/observation/types.ts"), "utf8");
