...
await mkdir(project); let store: SqliteOperatingStore | null = null;
try {
  const contracts = await createOperatingContracts(path.join(root, "contracts"));
  const database = path.join(temporary, "data", "kovacs.db"); store = await SqliteOperatingStore.create(database, contracts);
