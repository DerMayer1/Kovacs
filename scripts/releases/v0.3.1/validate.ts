...
const required = ["docs/v0.3.1/00_HARDENING_CHARTER.md", "docs/v0.3.1/01_MIGRATION_AND_RECOVERY.md", "docs/v0.3.1/02_PILOT_PROTOCOL.md", "test/operating-system/hardening.test.ts", "scripts/releases/v0.3.1/smoke.ts", "scripts/releases/v0.3.1/live.ts"];
...
const sources = await Promise.all([
  "src/infrastructure/persistence/sqlite-operating-store.ts", "src/application/operating-system/operating-system.ts", "src/core/operating-system/types.ts", "src/application/observation/observation-controller.ts", "src/interfaces/desktop/main.ts", "ui/desktop/preload.cjs", "ui/desktop/renderer.js", "test/operating-system/hardening.test.ts", "README.md",
...
record("H02", store.includes("ensureColumn") && ["0.3.1", "0.3.2", "0.3.3"].some((version) => store.includes(`schema_version', '${version}`)) && tests.includes("recover"), "additive schema migration and restart recovery are implemented");
...
record("H13", existsSync(path.join(root, "scripts/releases/v0.3.1/live.ts")) && readme.includes("release:validate"), "real Codex acceptance is explicit and separate from the no-cost automated gate");
