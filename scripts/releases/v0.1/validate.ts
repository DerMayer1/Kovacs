...
import { createContractValidator, type ContractValidator } from "../../../src/infrastructure/contracts/coaching-contracts.js";
import { loadCoachingConfig } from "../../../src/infrastructure/config/coaching-config.js";
import { validateAssistance } from "../../../src/core/coaching/assistance.js";
import { buildPrompt } from "../../../src/application/coaching/prompt.js";
import { redactSecrets } from "../../../src/core/security/sensitive-content.js";
import type { ManualRequest, ProfileResponse } from "../../../src/core/coaching/types.js";
...
const config = loadCoachingConfig(root);
...
const required = [
  ...
  "src/infrastructure/codex/codex-exec-gateway.ts", "src/application/coaching/coaching-service.ts", "src/interfaces/cli/terminal.ts", "benchmarks/v0.1/cases.json", "benchmarks/v0.1/RUBRIC.md",
];
...
metric("M07", (await readFile(path.join(root, "src", "core", "security", "sensitive-content.ts"), "utf8")).includes('sensitivity === "restricted"'), "restricted sensitivity has a pre-gateway block");
...
metric("M10", config.contextCharacterBudget > 0 && (await readFile(path.join(root, "src", "application", "coaching", "project-context.ts"), "utf8")).includes("remaining"), `hard budget configured at ${config.contextCharacterBudget} characters`);
...
const runtimeTests = await readFile(path.join(root, "test", "coaching", "coaching-runtime.test.ts"), "utf8");
...
metric("M14", runtimeTests.includes("service.status") && (await readFile(path.join(root, "src", "infrastructure", "persistence", "session-store.ts"), "utf8")).includes("session.events.push"), "session audit reconstruction covered");
...
const runtimeFiles = [
  "src/core/coaching/types.ts", "src/core/coaching/assistance.ts", "src/core/security/sensitive-content.ts",
  "src/application/coaching/prompt.ts", "src/application/coaching/project-context.ts", "src/application/coaching/coaching-service.ts",
  "src/infrastructure/codex/codex-exec-gateway.ts", "src/infrastructure/persistence/session-store.ts", "src/interfaces/cli/terminal.ts",
];
const runtimeSource = (await Promise.all(runtimeFiles.map((file) => readFile(path.join(root, file), "utf8")))).join("\n");
