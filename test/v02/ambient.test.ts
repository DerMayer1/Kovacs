...
import { authorizeWindow } from "../../src/v02/permissions.js";
import { frameDifference, isMeaningfulFrameChange } from "../../src/v02/change-detector.js";
import { automaticInterventionAllowed } from "../../src/v02/policy.js";
import type { ActiveWindowInfo, FrameCapture, WindowProbe } from "../../src/v02/types.js";
...
async function setup() {
  ...
  const contracts = await createAmbientContracts(path.join(root, "contracts"));
  const store = new AmbientStateStore(path.join(directory, "data"), contracts);
  ...
  const controller = new AmbientController(service as unknown as KovacsService, store, settings, windows, frames);
...
test("Observe Now reuses only a recent authorized window after the pet receives focus", async (t) => {
  ...
  const controller = new AmbientController(
    ...
    new AmbientStateStore(path.join(x.directory, "manual-data"), contracts),
...
test("application recovery always pauses a previously observing day", async (t) => {
  ...
  const contracts = await createAmbientContracts(path.join(root, "contracts"));
  const recovered = new AmbientController(x.service as unknown as KovacsService, new AmbientStateStore(path.join(x.directory, "data"), contracts), defaultAmbientSettings(), { getActiveWindow: async () => activeWindow }, { capture: async () => null });
