...
import { OperatingSystemController } from "../../src/application/operating-system/operating-system.js";
import { SqliteOperatingStore } from "../../src/infrastructure/persistence/sqlite-operating-store.js";
import type { CalibrationInput, DayProposal, OperatingPlanner, OperatingProfile, PlannerExecution, SetupInput, SetupProposal, WeekInput, WeekProposal } from "../../src/core/operating-system/types.js";
...
class FakePlanner implements OperatingPlanner {
  calls: string[] = [];
...
async function fixture() {
  ...
  const [ambientContracts, operatingContracts] = await Promise.all([createObservationContracts(path.join(root, "contracts")), createOperatingContracts(path.join(root, "contracts"))]);
  const store = await SqliteOperatingStore.create(path.join(directory, "data", "kovacs.db"), operatingContracts);
  const service = new FakeService(), planner = new FakePlanner();
  let operating: OperatingSystemController;
  const ambient = new ObservationController(service as unknown as KovacsService, new ObservationStateStore(path.join(directory, "ambient"), ambientContracts), { ...defaultAmbientSettings(), automatic_intervention_interval_ms: 30_000 }, { getActiveWindow: async () => ({ application: "Code.exe", title: "V0.3 test", windowId: 1 }) }, { capture: async () => ({ sample: Buffer.alloc(64, 180), png: Buffer.from("ephemeral-v03-frame") }) }, { operatingContext: () => operating.contextSummary(), onReasoningComplete: (telemetry) => operating.recordAmbientInvocation(telemetry) });
  operating = new OperatingSystemController(ambient, store, planner); await operating.initialize();
...
test("SQLite state survives restart while observation recovery remains paused", async (t) => {
  ...
  const contracts = await createOperatingContracts(path.join(root, "contracts")); const reopened = await SqliteOperatingStore.create(databasePath, contracts);
