...
import { normalizeCodexOutputSchema } from "../../src/infrastructure/codex/codex-exec-gateway.js";
import type { DayProposal, SetupInput, SetupProposal } from "../../src/core/operating-system/types.js";
...
async function fixture() {
  ...
  const store = await SqliteOperatingStore.create(database, contracts);
...
function startDay(store: SqliteOperatingStore, project: string) {
...
test("unfinished work and interrupted invocations recover without implicit observation", async (t) => {
  ...
  x.store.close();
  const reopened = await SqliteOperatingStore.create(x.database, x.contracts);
...
test("a V0.3.0 database receives additive migrations through V0.3.3", async (t) => {
  ...
  `); legacy.close();
  const contracts = await createOperatingContracts(path.join(root, "contracts")), migrated = await SqliteOperatingStore.create(database, contracts);
  t.after(async () => { migrated.close(); await rm(directory, { recursive: true, force: true }); });
  assert.equal(migrated.snapshot().recovery.schema_version_applied, "0.3.3");
...
test("feedback, scoped memory deletion, retention, backup, and export remain local", async (t) => {
  ...
  const exported = await readFile(backup.export, "utf8"); assert.match(exported, /"schema_version": "0.3.3"/); assert.doesNotMatch(exported, /"window_title":/);
});
...
test("legacy ambient window titles are scrubbed during recovery", async (t) => {
  ...
  const contracts = await createObservationContracts(path.join(root, "contracts")); const state = new ObservationStateStore(directory, contracts);
