...
import type { ObservationController } from "../../src/application/observation/observation-controller.js";
import { AmbientContextDecisionEngine } from "../../src/core/observation/intervention-decision.js";
import type { ActiveWindowInfo, AmbientLocalPerceptionResult, CapturedFrame } from "../../src/core/observation/types.js";
...
class CalibrationPlanner implements OperatingPlanner {
  refinementCalls = 0;
...
test("calibration corrections are versioned locally and clarification spends exactly one call", async (t) => {
  ...
  const ambient = { onUpdate: () => () => undefined, getState: () => null } as unknown as ObservationController;
  const controller = new OperatingSystemController(ambient, x.store, planner);
