...
import type { ActiveWindowInfo, CapturedFrame } from "../../src/core/observation/types.js";
...
test("ObservationController does not capture or attach an image when local perception is sufficient", async (t) => {
  ...
  const contracts = await createObservationContracts(path.join(root, "contracts")), service = new FakeService(); let captures = 0, contextEvents = 0, clears = 0;
  const controller = new ObservationController(service as unknown as KovacsService, new ObservationStateStore(path.join(directory, "ambient"), contracts),
